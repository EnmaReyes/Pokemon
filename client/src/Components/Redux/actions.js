...
export const getPokemonDetail = (id) => {
  return async function (dispatch) {
    const pokemons = (await axios.get(`http://localhost:3001/pokemon/${id}`))
      .data;
...
export const getPokemonsId = (id) => {
  return async function (dispatch) {
    const pokedex = (await axios.get(`http://localhost:3001/pokemon/${id}`))
      .data;
...
export const shearchByID = (id) => {
  return async function (dispatch) {
    try {
      const SearchID = (await axios.get(`http://localhost:3001/pokemon/:${id}`))
        .data;
      return dispatch({
        type: GET_POKEMON_ID,
        payload: SearchID,
